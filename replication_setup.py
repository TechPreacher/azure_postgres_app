...
def check_env_vars() -> bool:
    ...
    required_vars = [
        'AZURE_POSTGRES_PRIMARY_HOST', 'AZURE_POSTGRES_PRIMARY_USER', 
        'AZURE_POSTGRES_PRIMARY_PASSWORD', 'AZURE_POSTGRES_PRIMARY_SERVER_NAME',
        'AZURE_POSTGRES_REPLICA_HOST', 'AZURE_POSTGRES_REPLICA_USER',
        'AZURE_POSTGRES_REPLICA_PASSWORD', 'AZURE_POSTGRES_REPLICA_SERVER_NAME'
    ]
...
def check_logical_replication_settings(engine: Engine) -> bool:
    ...
    try:
        with engine.connect() as conn:
            ...
            replication_slots = conn.execute(text("SHOW max_replication_slots")).scalar()
    ...
    try:
        with primary_engine.connect() as conn:
            ...
            result = conn.execute(text(
                "SELECT COUNT(*) FROM pg_publication WHERE pubname = :name"
            ), {"name": publication_name}).scalar()
...
def create_subscription(replica_engine: Engine, publication_name: str) -> bool:
    ...
    try:
        ...
        with replica_engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
            # Check if subscription already exists
            result = conn.execute(text(
                "SELECT COUNT(*) FROM pg_subscription WHERE subname = :name"
            ), {"name": subscription_name}).scalar()
            ...
            connection_string = f"host={PRIMARY_DB_CONFIG['host']} dbname={PRIMARY_DB_CONFIG['database']} user={PRIMARY_DB_CONFIG['user']} password={PRIMARY_DB_CONFIG['password']} sslmode={PRIMARY_DB_CONFIG['sslmode']}"
    ...
    try:
        with replica_engine.connect() as conn:
            ...
            if stats:
                ...
                for row in stats:
                    print(f"Subscription: {row[0]}, State: {row[1]}, Relation: {row[2]}")
