...
def check_env_vars(db_type: DatabaseType) -> bool:
    ...
    if db_type in (DatabaseType.PRIMARY, DatabaseType.BOTH):
        primary_vars = ['AZURE_POSTGRES_PRIMARY_HOST', 'AZURE_POSTGRES_PRIMARY_USER', 
                         'AZURE_POSTGRES_PRIMARY_PASSWORD', 'AZURE_POSTGRES_PRIMARY_DB']
        
        primary_missing = [var for var in primary_vars if not os.environ.get(var)]
        if primary_missing:
            print(f"Error: Missing PRIMARY database environment variables: {', '.join(primary_missing)}")
    ...
    if db_type in (DatabaseType.REPLICA, DatabaseType.BOTH):
        replica_vars = ['AZURE_POSTGRES_REPLICA_HOST', 'AZURE_POSTGRES_REPLICA_USER', 
                         'AZURE_POSTGRES_REPLICA_PASSWORD', 'AZURE_POSTGRES_REPLICA_DB']
        
        replica_missing = [var for var in replica_vars if not os.environ.get(var)]
        if replica_missing:
            print(f"Error: Missing REPLICA database environment variables: {', '.join(replica_missing)}")
            print("Please create a .env file based on .env.example with your Azure PostgreSQL credentials.")
            sys.exit(1)
...
def connect_to_database(config: Dict[str, str], db_type_desc: str) -> Engine:
    ...
    try:
        ...
        connection_string = f"postgresql+psycopg2://{config['user']}:{config['password']}@{config['host']}/{config['database']}?sslmode={config['sslmode']}"
...
def load_sample_data(engine: Engine, is_primary: bool) -> None:
    ...
    try:
        ...
        if not is_primary:
            print("Skipping data import for replica database - data will come from replication")
            session.close()
            return
        ...
        print(f"Error: Sample data file not found at data/sample_data.json")
        sys.exit(1)
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            for (category,) in categories:
                count = session.query(func.count(Product.id)).filter(Product.category == category).scalar()
                avg_price = session.query(func.avg(Product.price)).filter(Product.category == category).scalar()
            ...
            print("This is expected for the replica database until replication is set up.")
...
def main() -> None:
    ...
    if db_type == DatabaseType.BOTH:
        ...
        print("You can now run 'python replication_setup.py' to configure replication between them.")
